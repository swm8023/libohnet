...
START_TEST(test_time)
{
	...
	fntime_to_str(get_catime(), buf2, 25);
	ck_assert_str_eq(buf1, buf2);
...
{
	ck_assert_int_eq(max_int(3, -3), 3);
	ck_assert_int_eq(max_int(3,  5), 5);
	ck_assert_int_eq(min_int(3, -3),-3);
	ck_assert_int_eq(min_int(3,  5), 3);
}
END_TEST
