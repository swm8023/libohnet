...
typedef struct _tag_listnode_t {
    struct _tag_list_t* _lst;
    struct _tag_listnode_t* _pre;
    struct _tag_listnode_t* _next;
    char _data[0];
} _listnode_t;
...
#define _LIST_SIZE(lst) ((lst)->_size)
#define _LIST_GUARD(lst) ((lst)->_guard)
...
#define _ITER_CONTAIN_LIST(iter) ((list_t*)((iter)._container))
