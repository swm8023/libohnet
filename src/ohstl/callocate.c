...
static void* _cmemory_pool_alloc(size_t);
static void* _cmemory_pool_realloc(void*, size_t);
static void _cmemory_pool_free(void*);

static void* _cmemory_sys_alloc(size_t);
static void* _cmemory_sys_realloc(void*, size_t);
static void _cmemory_sys_free(void*);

static _alloc_if _cmemory_pool_if = {
    _cmemory_pool_alloc,
    _cmemory_pool_realloc,
    _cmemory_pool_free,
};
static _alloc_if _cmemory_sys_if = {
    _cmemory_sys_alloc,
    _cmemory_sys_realloc,
    _cmemory_sys_free,
};
_alloc_if *_default_cmem_if = &_cmemory_sys_if;

void* _cmemory_sys_alloc(size_t size) {
    return ohmalloc(size);
}
void* _cmemory_sys_realloc(void* ptr, size_t size) {
    return ohrealloc(ptr, size);
}
void _cmemory_sys_free(void* ptr) {
    ohfree(ptr);
}
...
void *_cmemory_pool_realloc(void *ptr, size_t size) {
    ...
    void *newptr = _cmemory_pool_alloc(size);
    assert(newptr != NULL);
    memcpy((char*)newptr, (char*)ptr, upool->unitsize);
    _cmemory_pool_free(ptr);

    return newptr;
}

void _cmemory_pool_free(void *ptr) {
    assert(ptr != NULL);
...
_cmempool_t* _create_mempools() {
    ...
    pools = (_cmempool_t*)ohmalloc(pools_num * sizeof(_cmempool_t));
...
int _append_memblock(_cmempool_t *pool) {
    ...
    _cmemblock_t* block = (_cmemblock_t*)ohmalloc(realsize);
